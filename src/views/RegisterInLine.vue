...
<div id="register-in-line-container">
  ...
  <div id="digital-or-not-checkbox">
    <input v-model="studentInQueue.location" type="radio" id="school-queue-btn" value="true" name="queue-btn" v-on:click="campusChosen">
    <label id="school-queue-header" for="school-queue-btn">Campus</label>
    <input v-model="studentInQueue.location" type="radio" id="digital-queue-btn" value="false" name="queue-btn" v-on:click="digitalChosen">
  ...
  <div id="list-of-ovinger-wrapper">
    <div id="list-of-ovinger-container">
      <div id="list-of-ovinger" v-for="assigment in assignments" :key="assigment">
        <label id="oving-checkbox-header" class="oving-lable" value="fire" for="oving-checkbox">Øving {{assigment.number}}</label> <!--For å vise at en box er cheked så er det bare: const cb = document.querySelector('#oving-checkbox-header'); også cd.value ==true-->
        <input v-model="studentInQueue.assignmentNumber" :value=assigment.number type="radio" id="oving-checkbox" class="oving-checkbox" name="oving-checkbox">
      </div>
    </div>
  </div>
  <div id="approval-or-help-checkbox">
    <input v-model="studentInQueue.assessmentType" type="radio" id="approval-queue-btn" value="true" name="queue-btn-2">
    <label id="approval-queue-header" for="approval-queue-btn">Godkjenning</label>
    <input v-model="studentInQueue.assessmentType" type="radio" id="help-queue-btn" value="false" name="queue-btn-2">
  ...
  <div id="destination-on-campus" v-if="showCampusDetails">
    ...
    <input v-model="studentInQueue.campus" id="campus-input" class="destination-input">
    <br>
    <label id="building-label" class="destination-lable" for="building-input">Bygning:</label>
    <input v-model="studentInQueue.building" id="building-input" class="destination-input">
    <br>
    <label id="room-label" class="destination-lable" for="room-input">Rom:</label>
    <input v-model="studentInQueue.room" id="room-input" class="destination-input">
    <br>
    <label id="table-label" class="destination-lable" for="table-input">Bord:</label>
    <input v-model="studentInQueue.tableNumber" id="table-input" class="destination-input">
  </div>
  <div id="stand-in-line-wrapper">
  <button id="stand-in-queue-btn" v-on:click="sendQueueRegistration">Still i kø</button>
  </div>
</div>
  <Footer></Footer>
</template>
...
export default {
  ...
  methods : {
    ...
    getAssignmentsInCourse : async function(){
      await AXI.getAllAssignmentsInCourseForStudentAndIfApproved(this.$store.state.course.courseId, this.$store.state.userId).then(
          function (response) {
